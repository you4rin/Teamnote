// boj.kr/14555
// code based on https://github.com/koosaga/DeobureoMinkyuParty/blob/master/teamnote.pdf
#include<cstdio>
#include<cmath>
#include<vector>
#include<algorithm>
...
int main(){
	vector<ll> a,b,c,ret;
	a.resize(1<<19);b.resize(1<<19);c.resize(1<<19);
	int n,m;
	ll tmp,ans=0,add=0;
	scanf("%d %d",&n,&m);
	for(int i=0;i<n;++i){
	...
	for(int i=1;i<min(a.size(),ret.size());++i){
		add+=a[i]*ret[i];
		ans+=a[0]*a[i]*(a[i]-1)/2;
	}
	printf("%lld",ans+add/6);
}